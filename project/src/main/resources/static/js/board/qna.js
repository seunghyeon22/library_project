...
boardWriting.onclick = () => {
	if(principal == null){
		...
		location.href = "/book/qAndAWriting";
	}
}
...
function load(page) {
	$.ajax({
		type : 'GET',
		url : `/api/v1/qnaboard/search/${page}`,
		data : {
			"page" : page,
...
function getBoardList(data){
	...
	for(let i = 0; i < data.length; i++){
		tableStr += `
			<tr>
                <td>${data[i].qna_code}</td>
                <td class="tit"><a href="/book/qAndABoard/${data[i].qna_code}">${data[i].qna_title}</a></td>
                <td>${data[i].qna_username}</td>
