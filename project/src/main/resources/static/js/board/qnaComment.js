 ...
 function load(page) {
	let url = `/api/v1/qnacomment/list/${qnacode}?page=${page}`
	...
	.then(result => {
		...
		createPageNumber(result[0].qnacommentTotalCount);
	})
	.catch(error => {
		console.log(error)
	})
...
function createPageNumber(data) {
 ...
 function getCommentList(data) {
	...
	for(let i = 0; i < data.length; i++){
		tableStr += `
			<tr>
                <td class="name">${data[i].qnacomment_username}</td>
                <td>${data[i].qnacomment_contents}</td>
...
function comment() {
	$.ajax({
		type : 	'POST',
		url : `/api/v1/qnacomment/post`,
		contentType: "application/json",
		data : JSON.stringify({
			qna_code: qnacode,
			qnacomment_username: principal.username,
			qnacomment_contents: commentTextarea.value
		}),
		dataType : "JSON",
