...
function getBoardlist(data) {
	boardView.innerHTML += `
        ...
        <div class="info">
        	<dl>
	            <dt>번호</dt>
	            <dd>${data[0].qna_code}</dd>
	        </dl>
	        <dl>
	            <dt>글쓴이</dt>
	            <dd>${data[0].qna_username}</dd>
	        </dl>
	        <dl>
	            <dt>작성일</dt>
...
function getButton(data) {
	if(principalUsername != null){
		if(principalUsername.username == data[0].qna_username){
			btWrap.innerHTML = `
				<button type="button" class="update_btn">수정</button>
				<button type="button" class="delete_btn">삭제</button>
				<button type="button" onclick="location.href = '/book/qAndA/'">목록</button>
			`
...
function boardBtns() {
	...
	updateBtn.onclick = () => {
		location.href = "/book/qAndA/" + qnacode;
	}
